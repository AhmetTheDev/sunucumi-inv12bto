...
exports.run = async (client, message, args) => {
 ...
 if(!['773291214673149962', '773291177730113566', '773291177092186112'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
let tag = "⍱"
const kayıtlı = message.guild.roles.cache.find(r => r.id === '773291238173573131')
const kayıtlı2 = message.guild.roles.cache.find(r => r.id === '773291239038255124')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '773291242049241128')
...
.setTitle(`Kayıt İşlemi Tamamlandı`)
    ...
    .addField(`Verilen Rol:`, `<@&${kayıtlı.id}>, <@&${kayıtlı2.id}> Rolleri Verildi`) 
...
.setFooter(`VALOROUS <3 TITAN`)
.setColor('GREEN')
client.channels.cache.get('773291347859603506', '773291353156616202').send(embed)
